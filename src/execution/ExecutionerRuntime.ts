...
import {
  ...
  instrumentFromBinanceSymbol,
  isBinanceOrderTestMode
} from "./BinanceExecutionAdapter";
import {
...
} from "./HyperliquidWire";
import { IntentIdempotencyLedger } from "./IntentIdempotency";
import {
  finiteNumber,
  isRecord,
  numberField,
...
import { evaluateExecutionRisk, isInventoryHedgeIntent } from "./RiskGuards";
import { exchangeSecret, exchangeSecretWithSource } from "./SecretResolver";
import { buildShadowRestingQuoteReport, estimateShadowFees } from "./ShadowExecutionReports";
import { buildTwapSlices, fillRatio, type TwapSlice } from "./TwapSlicing";
...
export async function executeIntent(
  ...
  const isPostOnlyLimit = intent.orderType === "LIMIT" && intent.postOnly;
  if (!isPostOnlyLimit && !inventoryHedge) {
...
export async function listOpenOrders(env: Env, logger: Logger): Promise<Response> {
...
export async function executionerDiagnostics(env: Env): Promise<Response> {
...
export async function getAccountBalance(env: Env, logger: Logger): Promise<Response> {
  if (resolveAdapter(env) === "binance-us") {
...
async function cancelBinanceOpenOrders(env: Env, instrumentCode?: string): Promise<Response> {
  ...
  const results: { orderId: string; instrumentCode: string; status: number; ok: boolean }[] = [];
...
async function prepareHyperliquidCancelRequest(
  env: Env,
  cancels: { instrumentCode: string; orderId: string }[]
): Promise<PreparedExchangeRequest> {
  const nonce = Date.now();
  const expiresAfter = hyperliquidExpiresAfter(env, nonce);
  ...
  const action: JsonRecord =
    "o" in cancelWires[0]
      ? { type: "cancel", cancels: cancelWires }
      : { type: "cancelByCloid", cancels: cancelWires };
  const signingStartedAt = performance.now();
  const signature = await SignatureEngine.signHyperliquidL1Action({
    secret: agentSecret,
    action,
...
function hyperliquidTif(env: Env, intent: TradeIntent): "Alo" | "Ioc" | "Gtc" {
  ...
  if (intent.postOnly || intent.postOnly === undefined) {
    return "Alo";
  }
